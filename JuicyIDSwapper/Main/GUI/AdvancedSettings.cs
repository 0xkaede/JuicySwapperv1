...
namespace JuicyIDSwapper.Main.GUI
{
    public partial class AdvancedSettings : Form
    {
        ...
        private void restartButton_Click(object sender, EventArgs e)
        {
            ...
            DialogResult restart = MessageBox.Show("Are you sure you want to restart?", "Juicy ID Swapper", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        {
            int num = 0;
            if (File.Exists(Properties.Settings.Default.pakPath + $"\\pakchunk0-WindowsClient.pak"))
            {
                num++;
            }
            if (num == 3)
            {
                MessageBox.Show("Successfully found pakchunk0-WindowsClient.pak!", "Juicy ID Swapper", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            if (num < 3)
            {
                if (num == 0 && File.Exists(Properties.Settings.Default.pakPath + $"\\pakchunk10-WindowsClient.pak"))
                {
                    MessageBox.Show("You have selected the right paks folder, however one of your pak files seem to be missing. Please verify your game via. the Epic Games Launcher.", "Juicy ID Swapper", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                MessageBox.Show("Could not find pak files! Please select the correct directory using the pak file selector", "Juicy ID Swapper", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
